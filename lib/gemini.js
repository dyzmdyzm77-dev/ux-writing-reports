...
export async function callGemini(apiKey, prompt, schema) {
  // 개인 키가 없으면 서버 공용 키(Vercel 환경변수 GEMINI_API_KEY)로 대신 호출한다.
  // → 키를 등록하지 않은 팀원도 플러그인만 켜면 AI 추천을 쓸 수 있다 (공용 무료 한도를 팀이 공유).
  //   개인 키를 등록한 사람은 자기 키가 우선이라 공용 한도를 갉아먹지 않는다.
  ...
  const first = await callGeminiModel(key, GEMINI_MODEL, prompt, schema);
  if (!first.retryable) return first;
  const second = await callGeminiModel(key, GEMINI_FALLBACK_MODEL, prompt, schema);
